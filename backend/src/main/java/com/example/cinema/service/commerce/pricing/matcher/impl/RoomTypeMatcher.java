...
@Component
public class RoomTypeMatcher implements IConditionMatcher {
    @Override public PricingConditionType getSupportedType() { return PricingConditionType.ROOM_TYPE; }
    @Override public boolean matches(PricingCondition cond, Showtime showtime, Seat seat, Customer customer) {
        return cond.getValue().equals(showtime.getRoom().getRoomType().getId());
    }
}
